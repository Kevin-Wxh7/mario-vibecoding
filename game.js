...
const COYOTE_TIME_MS = 100; // Small jump grace window after leaving ground
const JUMP_BUFFER_MS = 120; // Small input buffer before landing
...
class LevelScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.movingPlatform = createMovingPlatform(this, level.movingPlatform);
  ...
  update() {
    ...
    // Jump buffer: remember jump input briefly, then consume on valid jump frame
    if (Phaser.Input.Keyboard.JustDown(this.cursors.up)) {
      this.jumpBufferUntil = this.time.now + JUMP_BUFFER_MS;
    }

    // Jump when grounded/coyote and jump is buffered
    const canJump = onGround || (this.time.now - this.lastGroundedTime <= COYOTE_TIME_MS);
    const hasBufferedJump = this.time.now <= this.jumpBufferUntil;
    if (hasBufferedJump && canJump) {
      this.player.setVelocityY(-JUMP.jumpVelocity);
      this.lastGroundedTime = -Infinity; // Consume coyote window after jump
      this.jumpBufferUntil = -Infinity;  // Consume jump buffer after jump
    }
    ...
    // Moving platform patrol movement
    if (this.movingPlatform && this.movingPlatform.active) {
      if (this.movingPlatform.x <= this.level.movingPlatform.minX) {
        this.movingPlatform.setVelocityX(this.level.movingPlatform.speed);
      } else if (this.movingPlatform.x >= this.level.movingPlatform.maxX) {
        this.movingPlatform.setVelocityX(-this.level.movingPlatform.speed);
      }
    }
  }
...
function createMovingPlatform(scene, movingPlatformData) {
  if (!movingPlatformData) return null;

  const platform = scene.physics.add.image(movingPlatformData.x, movingPlatformData.y, ASSETS.platform)
    .setDisplaySize(movingPlatformData.w, movingPlatformData.h);

  platform.setImmovable(true);
  platform.body.allowGravity = false;
  if (platform.setPushable) platform.setPushable(false);
  platform.setVelocityX(movingPlatformData.speed);
  return platform;
}
